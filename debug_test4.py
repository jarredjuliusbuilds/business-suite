...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///test_debug5.db'

with app.app_context():
    db.create_all()
    
    user = User(email='debug5@test.com', password_hash='hash')
    db.session.add(user)
    db.session.commit()
    ...
    app.test_client().open('/')  # ensure template globals work
    
    client = app.test_client()
    
    # Log in using session_transaction
    ...
    if r.location:
        ...
        print('Dashboard has traceback:', 'Traceback' in text or 'Error' in text)
    ...
    if r6.location:
        ...
        print('Invoices has traceback:', 'Traceback' in text7 or 'Error' in text7)